...
async def get_resumen_ventas_perdidas(
    ...
    """Resumen de ventas perdidas del mes actual por motivo"""
    if not current_user.sucursal_id:
        raise HTTPException(status_code=400, detail="Usuario sin sucursal asignada")
    ...
    query = text("""
        SELECT
            ...
            SUM(CASE WHEN motivo = 'stock' OR (motivo IS NULL AND NOT es_producto_nuevo) THEN 1 ELSE 0 END) as falta_stock,
            SUM(CASE WHEN motivo = 'precio' THEN 1 ELSE 0 END) as por_precio,
            SUM(CASE WHEN motivo = 'otro' THEN 1 ELSE 0 END) as otros,
            SUM(CASE WHEN motivo = 'nuevo' OR es_producto_nuevo THEN 1 ELSE 0 END) as productos_nuevos
    ...
    return {
        ...
        "por_precio": result.por_precio or 0,
        "otros": result.otros or 0,
